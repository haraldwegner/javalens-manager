...
//! agent at JVM start. JavaLens *is* JDT/ECJ, so for its analysis tools to
...
//! inside the JavaLens product (Option B — version-locked to the product's own
...
/// Locate the `lombok.jar` shipped inside the JavaLens product, given the
...
mod tests {
    ...
    fn javaagent_arg_requires_both_lombok_and_jar() {
        let product = tmp();
        let jar = product.join("javalens.jar");
