...
**Beta (v0.15.0)**: `javalens-manager` is a fully functional desktop application on Linux (x86_64 and aarch64) and macOS (Apple Silicon only — unsigned, Gatekeeper bypass required). It supports named workspaces of multiple Java projects (each running as one shared JavaLens MCP service), live `workspace.json`-driven reconciliation, automatic fork-runtime download/update, and one-click deploy of MCP entries into Cursor / Claude Desktop / Antigravity / IntelliJ-style configs. The system-tray menu drives per-workspace lifecycle without opening the window. As of v0.15.0 the manager hosts ONE resident javalens JVM per workspace and writes URL endpoints into client configs — N MCP clients × M workspaces collapse to **M JVMs total** (closes the 30 GB leak from v0.14.x stdio-per-client). Intel Macs aren't in scope (Apple Silicon only since v0.14.0). Windows builds (ARM + x64) ship in Sprint 16 / v0.16.0. Broader QA and cross-platform testing continue before a stable 1.0.
